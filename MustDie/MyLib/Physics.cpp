...
MyLib::Physics::Physics(int stageCollisionHandle)
{
...
void MyLib::Physics::Update()
{
	...
	for (auto& item : m_collidables)
	{
		if (item->GetTag() == GameObjectTag::Player || item->GetTag() == GameObjectTag::Enemy)
		{
			...
			{
				auto tes = dynamic_cast<MyLib::ColliderDataSphere*> (col.get())->m_radius;
				m_hitDim = MV1CollCheck_Sphere(m_stageCollisionHandle, -1, item->rigidbody.GetNextPosVECTOR(), tes);
			}
		}
		else
		{
...
void MyLib::Physics::FixPositionWithWall(std::shared_ptr<Collidable>& col)
{
	...
	if (col->rigidbody.GetDir().Size() != 0.0f)
	{
		...
		for (int i = 0; i < m_wallNum; i++)
		{
			...
			{
				// それを移動前の座標に足したものを新たな座標とする
				col->rigidbody.SetNextPos(col->rigidbody.GetNextPos() + SlideVec);
			}
