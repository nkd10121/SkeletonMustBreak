...
EnemyManager::EnemyManager() :
	m_deathEnemyNum(0)
{
	temp_enemyHandle = MV1LoadModel("data/model/SkeletonWarrior.mv1");
		...
		for (auto& generate : m_pGenerateInfo)
		{
			...
			if (generate->appearPhase == thisPhaseName && generate->appearFrame == thisPhaseCount)
			{
				if (!generate->isCreated)
				{
					...
					std::vector<MyLib::Vec3> pos;
					pos.push_back(MyLib::Vec3(0.0f, 0.0f, 50.0f));
					pos.push_back(MyLib::Vec3(10.0f, 0.0f, 10.0f));
					pos.push_back(MyLib::Vec3(-10.0f, 0.0f, -10.0f));
					pos.push_back(MyLib::Vec3(0.0f, 0.0f, -50.0f));
