...
WeaponBase::~WeaponBase()
{
	MV1DeleteModel(m_weaponModelHandle);
	MV1DeleteModel(m_ownerModelHandle);
}

void WeaponBase::Init(int weaponModelHandle, int ownerModelHandle, const TCHAR* franeName,float modelSize)
{
	m_weaponModelHandle = weaponModelHandle;
...
void WeaponBase::Update(MyLib::Vec3 moveVec)
{
	...
	MV1SetMatrix(m_weaponModelHandle, m_mat);
...
{
	MV1DrawModel(m_weaponModelHandle);
}

void WeaponBase::CollisionInit(std::shared_ptr<MyLib::Physics> physics)
{
...
void WeaponBase::CollisionEnd(std::shared_ptr<MyLib::Physics> physics)
{
	if (m_isCollisionOn)
	{
		Finalize(physics);
